...
class LoraInjectedLinear(nn.Module):
    ...
    def forward(self, input):
        ...
        down = self.lora_down(input.to(dtype))
        up= self.lora_up(down).to(ori_dtype)
        return self.linear(input) + up * self.scale
...
def inject_trainable_lora(
    ...
    for _module, name, _child_module in _find_modules(
        ...
        _tmp.to(_child_module.weight.device)
        #print(_child_module.weight.device)
        ...
        if loras != None:
            lora_up = loras.pop(0).type(torch.float32)
            lora_down = loras.pop(0).type(torch.float32)
